    ...
    def search(self, name: str = None, date: datetime.datetime = None) -> list:
        ...
        for i in data:
            if i["name"] == name or i["date"] == date.strftime("%Y-%m-%d %H:%M"):
                return_list.append(Remind.import_from_dict(i))
        return return_list
    

"""
if __name__ == "__main__":
    remind = Remind(name = "テスト", date = datetime.datetime.now())
    #remind.edit_date(in_date="2h")
    remind_registory = RemindRegistory(f"{Path(__file__).parent}/json/reminder.json")
    remind_registory.add(remind)
    remind_list = remind_registory.search(date=datetime.datetime.now())
    for i in remind_list:
        print(i.name)
        remind_registory.remove(i)
"""